...
| [Возможности](#возможности) | Привязка, NLP, помощь, «молчи», gRPC |
...
Поведение части служебных команд **отличается** от Salute — см. [«молчи» / «помолчи»](#служебная-команда-молчи--помолчи).
...
### Служебная команда «молчи» / «помолчи»

Распознаётся локально (без CVC): «молчи», «помолчи», «замолчи», «тише».

| Платформа | Поведение |
| --------- | --------- |
| **Salute** | Навык остаётся активным (`finished: false`), но **микрофон выключается** (`auto_listening: false`). Следующая реплика не уходит в навык, пока пользователь снова не обратится к нему. Это **пауза**, не выход. |
| **Alice** | В [API Яндекс Диалогов](https://yandex.ru/dev/dialogs/alice/doc/ru/response) **нет аналога** `auto_listening`. При `end_session: false` (текущее поведение) Алиса **продолжает слушать** следующую фразу в рамках сессии навыка. `end_session: true` — это **полный выход** из навыка (как «хватит» / «стоп»), а не пауза как в Salute. **Один в один поведение Salute на Алисе недоступно** на уровне протокола. |

Не путать с командой роботу **«хватит»** / **«вставай»** (`dismiss`) — это управление позой панды, а не режим диалога.

Подробнее о протоколе Alice: [запуск и выход](https://yandex.ru/dev/dialogs/alice/doc/ru/activation), [хранение состояния](https://yandex.ru/dev/dialogs/alice/doc/ru/session-persistence).
